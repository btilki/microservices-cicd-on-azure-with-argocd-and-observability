...
6. **Azure DevOps — pipeline source (control repository)** — Each pipeline in Azure DevOps is bound to the **GitHub repository** you picked when the pipeline was created (this is the “get sources” / **control repository** for `checkout: self`). After you **rename or move** the repo on GitHub, update **every** pipeline so it still points at the right GitHub URL and branch (`main`). See [pipelines/README.md](pipelines/README.md) → **Azure DevOps pipeline source after a GitHub rename**.
