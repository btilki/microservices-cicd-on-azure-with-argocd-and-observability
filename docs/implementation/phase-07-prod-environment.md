...
| Team | Purpose |
|------|---------|
| `YOUR_ORG/prod-gitops-approvers` | Primary code owners for `gitops/envs/prod/**` and `gitops/apps/prod/**` |
| `YOUR_ORG/prod-gitops-secondary` | Second reviewer / cab-style approval |

Root **`CODEOWNERS`** assigns both teams to those paths. Replace org/team slugs if your GitHub org is not `YOUR_ORG`.
...
1. **External HTTP checks** (expect **200** / **301** / **302**, not **5xx**):
   ...
   Optional TLS sanity:
   ```bash
   curl -I https://boutique.example.com/ 2>&1 | head -20
   ```

2. **App journey in browser:** open **https://boutique.example.com/** — load main flows (browse, cart if applicable). Watch devtools **Network** for failed calls to upstream gRPC services.
